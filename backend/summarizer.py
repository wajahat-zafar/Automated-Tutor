...
class Summarization:
    def __init__(self, text_ids):
        ...
        self.model = BartForConditionalGeneration.from_pretrained('F:\\Desktop\\fyp_code\\backend\\model_cnn')

    def summarize(self):
        summ_ids = self.model.generate(self.text_ids, num_beams=4, length_penalty=2.0, max_length=900, min_length=500, no_repeat_ngram_size=3, early_stopping=False)
        return summ_ids
        
class PreProcessor:
    def __init__(self,text):
        self.text = text
        self.tokenizer = BartTokenizer.from_pretrained('F:\\Desktop\\fyp_code\\backend\\model_cnn')

    def tokenize(self):
        text_input_ids = self.tokenizer.batch_encode_plus([self.text], return_tensors='pt', max_length=1024, truncation=True)['input_ids'].to(torch_device)
    ...
    def token(self):
        ...
        for nested in nested:
          ...
          summary_ids = self.model.to(torch_device).generate(input_tokenized, num_beams=4, no_repeat_ngram_size=3, early_stopping=False,
                                            length_penalty=3.0,
##                                            min_length=20,
                                            max_length=20)
    ...
    def decoder(self, summ_ids):
        summary_txt = self.tokenizer.decode(summ_ids.squeeze(), skip_special_tokens=True)
        return summary_txt
