...
class Data_scraper:
    ...
    def main(self):
        ...
        for topics_links in left_menu:
            ...
            self.dct[headings[counter]] = lst
            counter += 1

        for k in list(self.dct):
            for x in self.dct[k]:
                if 'Tutorial' in x:
                    self.dct[k].remove(x)
            if 'MCQ' in k:
...
# data.main()
# data.subtopics()
# data.scrapedata("Transport Layer")
