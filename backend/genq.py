...
class InputProcess:
    ...
    def __init__(self):
        ...
        self.possible_tags = set(['VBD', 'VBG', 'VBN', 'VB', 'VBZ'])
        self.tags = []
        self.text = ""
        self.question = None
    ...
    def preprocess_text(self, text):
        ...
        for elem in re.split(r'\.|,\n|\?|—|,|:', text):
            ...
            if sentence != '':
                self.text = re.sub(r'”|“|’|"', '', sentence)
                self.text = self.text.lower()
                self.tags = TextBlob(self.text).tags
                self.generate_q()
                if self.question:
                    quest.add(self.question)
                    dit[self.question] = self.text
        for i in quest:
            if len(i) > 35 and len(i) < 100:
                quest2.add(i)
##        return list(quest2)
        output = random.sample(list(quest2), 10)
        return output

    def generate_questions(self, item, index):
        if len(item) <= index + 1:
            return

        elif item[index + 1][1] not in self.possible_tags:
            return
        self.question = 'What '
        if item[index + 1][1] in ['VBD', 'VBG', 'VBN', 'VB', 'VBZ']:
            self.question = 'Who '
        if item[index][1] in ['PRP$']:
            self.question = 'Whose '
        if item[index][0] in ['it']:
            self.question = 'What '
    ...
    def generate_q(self):
        for index, item in enumerate(self.tags):
            if item[1] in ['NN', 'NNS', 'PRP', 'NNP', 'NNPS', 'PRP$']:
                self.generate_questions(self.tags, index)
