...
class Data_scraper:
    ...
    def scrapedata(self, select):
        ...
        # options.add_argument("user-data-dir=C:/Users/rafay/AppData/Local/Google/Chrome/User Data")
    ...
    # print(soup.prettify())
        ...
        def tutorial_point():
            ...
            for all_text in new_big_template.find_all(srr):
                if all_text.text not in nn:
                    nn.append(all_text.text)
                    # print(all_text.text)
                    # print('\n')
        ...
        def techtarget():
            ...
            try:
                search_kw = f'{og_topic_name} :www.techtarget.com'
                ...
                # driver7.close()
                # driver7.close()
                req = Request(nlink, headers={'User-Agent': 'Mozilla/5.0'})
                soup3 = BeautifulSoup(req, 'lxml')
                # print(soup2.prettify())
                ...
                driver97 = webdriver.Chrome(
                    options=options, executable_path=PATH)
                driver97.get('https://www.google.com')
                search_kw = f'"{og_topic_name}" :www.techtarget.com'
                search = driver97.find_element_by_name('q')
                search.send_keys(search_kw)
                search.send_keys(Keys.RETURN)

                soup3 = BeautifulSoup(driver97.page_source, 'lxml')
                google_topic_link = soup3.find('div', {"class": "v7W49e"})
                ...
                # try:
                #     element_present = EC.presence_of_element_located((By.XPATH, '//*[@id="content-body"]/section[1]/h3'))
                #     WebDriverWait(driver97, 3).until(element_present)
                # except:
                #     print("Timed out waiting for page to load")

                # source = driver97.page_source
                #
                # driver97.close()
                # driver97.close()
                # req = Request(nlink, headers={'User-Agent': 'Mozilla/5.0'})
                ...
                html = response.text
                soup3 = BeautifulSoup(html, 'html.parser')
                # soup3 = BeautifulSoup(urlopen(nlink))
                # print(soup3)
                # soup3 = BeautifulSoup(nlink, 'lxml')
                # print(soup2.prettify())
                new_big_template3 = soup3.find('section', id='content-body')
                try:
                    bb = soup3.find('h3', class_='section-title')
...
# data.main()
# data.subtopics()
##data.scrapedata("Network Security")
##final_output = f"{data.output1}\n{data.output2}\n{data.output3}\n{data.output4}\n{data.output5}\n{data.output6}\n"
##print('\nthis is\n',final_output)
